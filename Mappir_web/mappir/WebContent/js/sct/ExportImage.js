...
OpenLayers.Control.ExportMap = OpenLayers.Class(OpenLayers.Control, {
    ...
    trigger: function(canvas) {
        ...
        var tmp = this.map.getLayersByName('Precipitación');
        if (tmp !== undefined && tmp.length > 0) {
        	this.map.removeLayer(tmp[0]);
        }
    ...
    drawLayer: function(layerCanvas) {
        if(undefined === layerCanvas){
            var imagenes = $(MappirMap.instance.mapa.div).find('img.olTileImage');
            for(var i  = 0; i < imagenes.length; i++){
                var imagen = imagenes[i];
                //imagen.crossOrigin = "anonymus";
                var position = $(imagen).position();
                try{
                    this.canvasContext.drawImage(imagen, position.left, position.top);
                } catch(e){
//                	console.log(e);
                    continue;
                }
